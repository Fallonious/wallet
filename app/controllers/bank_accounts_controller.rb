class BankAccountsController < ApplicationController
  ...
  def index
    @bank_accounts = BankAccount.order(:transaction_on).all
    @total_amount = BankAccount.total
    @numberoftransactions = @bank_accounts.length
    if @total_amount < 0
      @repremand = "Put the shoes down and back away slowly. You don't have the funds."
    end
  end
  ...
  private
    ...
    def bank_account_params
      params.require(:bank_account).permit(:transaction_on, :item_description, :transaction_amount)
    end
end
